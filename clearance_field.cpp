...
namespace clearance_field
{
	...
	LPAClearanceFieldAlgorithm::LPAClearanceFieldAlgorithm(int w, int h, int u,
		...
		: w(w), h(h), u(u), isObstacle(isObstacle), predecessorsVisitor(predecessorsVisitor), successorVisitor(successorVisitor)
	{
		g.resize(w, std::vector<int>(h, inf));
		rhs.resize(w, std::vector<int>(h, inf));
	}

	LPAClearanceFieldAlgorithm::K LPAClearanceFieldAlgorithm::ComputeK(int x, int y) const
	{
		return { std::min(g[x][y], rhs[x][y]), x, y };
	}

	int LPAClearanceFieldAlgorithm::Get(int x, int y) const
	{
		return g[x][y];
	}
	...
	int LPAClearanceFieldAlgorithm::Compute()
	{
		...
		while (q.size())
		{
			...
			if (updatedCellVisitor != nullptr && x < w && y < h)
				updatedCellVisitor(x, y);
			...
			if (g[x][y] != inf && g[x][y] == rhs[x][y] && g[x][y] >= u)
				continue;
	...
	ClearanceFieldBase::ClearanceFieldBase(int w, int h, int u, int costUnit, int diagonalCostUnit,
		ObstacleChecker isObstacle)
		: w(w), h(h), u(u), costUnit(costUnit), diagonalCostUnit(diagonalCostUnit), originalIsObstacle(isObstacle)
	{
		// initial directions.
		for (int i = 0; i < 8; ++i)
		{
	...
	int TrueClearanceField::Get(int x, int y) const
	{
		return lpa->Get(x, y);
	}
	...
	int BrushfireClearanceField::Get(int x, int y) const
	{
		return lpa->Get(x + 1, y + 1);
	}
	void BrushfireClearanceField::Update(int x, int y)
	{
		lpa->Update(x + 1, y + 1);
	}
